...
export function ContactForm() {
  ...
  const validate = (): boolean => {
    ...
    if (!form.first_name.trim() || form.first_name.trim().length < 2) {
      e.first_name = t.form.errorRequired;
    }
    ...
    if (form.phone && !/^[+\d\s\-()]{7,20}$/.test(form.phone)) {
      e.phone = t.form.errorPhone;
    }

    if (form.message && form.message.trim().length > 0 && form.message.trim().length < 10) {
      e.message = t.form.errorMessage;
    }

    if (!form.consent) {
      e.consent = t.form.errorConsent;
    }
  ...
  if (status === 'success') {
    return (
      <div className="rounded-2xl border border-cyan-400/20 bg-cyan-500/5 p-8 text-center">
        ...
        <p className="text-white text-lg leading-relaxed mb-6">{t.form.success}</p>
        <button
    ...
    >
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm text-gray-300 mb-1.5">
            {t.form.firstName} <span className="text-cyan-400">*</span>
          </label>
          <input
            type="text"
      ...
      <div>
        ...
        <select
          ...
          disabled={status === 'loading'}
        >
          <option value="" className="bg-[#0a0e14]">{t.form.servicePlaceholder}</option>
          {services.map((s) => (
            <option key={s.id} value={s[titleKey] ?? s.title_en ?? ''} className="bg-[#0a0e14]">
              {s[titleKey] ?? s.title_en}
            </option>
          ))}
        </select>
      </div>
      ...
      <div>
        <label className="flex items-start gap-3 cursor-pointer">
          <input
            ...
            disabled={status === 'loading'}
          />
          <span className="text-sm text-gray-400">{t.form.consent}</span>
        </label>
